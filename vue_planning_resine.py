...
def valider_horaire_lancement(dt, heure_deb, heure_fin):
    """ 
    Trouve le moment où un opérateur est présent pour lancer la production résine.
    Si on dépasse l'heure de fin, on repousse au matin ouvré suivant.
    """
    while True:
        if est_chome(dt):
...
class OngletPlanning(ctk.CTkFrame):
    def __init__(self, master, fonction_rafraichir_global=None):
        ...
        # ==========================================
        # EN-TÊTE & PARAMÈTRES HORAIRES
        # ==========================================
        header_frame = ctk.CTkFrame(self, fg_color="transparent")
        header_frame.pack(fill="x", pady=(0, 5))

        ctk.CTkLabel(header_frame, text="📅 Planning Gantt Résine", font=ctk.CTkFont(size=26, weight="bold")).pack(side="left")
        ...
        # ==========================================
        # BARRE DE NAVIGATION DISCRÈTE (FLÈCHES)
        # ==========================================
    ...
    def appliquer_scroll(self):
        ...
        pixels = (target_dt - self.heure_debut_planning).total_seconds() / 60.0 * 2.5
    ...
    # ==========================================
    # GESTION BASE DE DONNÉES ATELIER RESINE
    # ==========================================
    def charger_horaires_ui(self):
        conn = sqlite3.connect("atelier_resine.db", timeout=5.0)
        cursor = conn.cursor()
        try:
            cursor.execute("""
    ...
    def recuperer_horaires_atelier(self):
        conn = sqlite3.connect("atelier_resine.db", timeout=30.0)
        c = conn.cursor()
        try:
    ...
    def charger_taches(self):
        ...
        try:
            ...
            # Historique de la journée pour l'affichage visuel
    ...
    def rafraichir_planning(self):
        ...
        if not taches:
            ctk.CTkLabel(self.main_scroll_y, text="🎉 L'atelier résine est libre. Aucune pièce en attente.", font=ctk.CTkFont(slant="italic", size=14), text_color="gray50").pack(pady=40)
            return
        ...
        # 1. Éclatement sécurisé en pièces unitaires
        pieces_a_planifier = []
        for t in taches:
            ...
            for piece_index in range(qte):
                ...
                # REPLI (FALLBACK) DE COMPATIBILITÉ MACHINES
                allowed = []
        ...
        # 2. Assignation chronologique (Prochaine pièce disponible & optimisation nuit)
        while pieces_a_planifier:
            ...
            for m_idx, mach in enumerate(machines):
                ...
                # Détection de la fenêtre de fin de journée pour optimisation nuit (2h avant départ)
                ...
                def sort_key(p_idx):
                    ...
                    if is_night_window:
                        # Priorité absolue aux pièces les plus longues pendant la nuit (-duree)
        ...
        # ==========================================
        # AUTO-ARCHIVAGE RESINE À 100%
        # ==========================================
        self.auto_archiver_termines(self.dernier_gantt)

        # ==========================================
        # RENDU DU GRAPHIQUE GANTT
        # ==========================================
    ...
    # ==========================================
    # GESTION PRODUCTION RESINE
    # ==========================================
    def auto_archiver_termines(self, gantt_donnees):
        """ Détection et archivage automatique à 100% dans atelier_resine.db """
        maintenant = datetime.now()
        groupes = {}
        ...
        conn = sqlite3.connect("atelier_resine.db")
        cursor = conn.cursor()
    ...
    def get_blocs_planifies(self):
        """ Alimente le tableau de suivi résine avec les pièces de 0% à 99% """
        maintenant = datetime.now()
        res = {}
        groupes = {}
